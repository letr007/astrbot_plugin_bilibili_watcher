...
class BilibiliAPI:
    ...
    async def fetch_user_likes(self, vmid: int) -> Optional[List[Dict]]:
        """
        异步获取用户的点赞视频列表
        ...
        try:
            logger.info(f"正在异步获取用户 {vmid} 的点赞视频...")
    ...
    async def fetch_user_info(self, vmid: int) -> Optional[Dict]:
        """
        异步获取用户基本信息
        ...
        try:
            logger.info(f"正在异步获取用户 {vmid} 的基本信息...")
    ...
    async def fetch_video_info(self, aid: int) -> Optional[Dict]:
        """
        异步获取视频详细信息
        ...
        try:
            logger.info(f"正在异步获取视频 {aid} 的详细信息...")
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        # 测试获取用户信息（使用一个已知的UID）
        test_uid = 1  # B站官方账号
        user_info = await api.fetch_user_info(test_uid)
        if user_info:
